...
const TAX_RATE = 0;               // set to 0.05 when GST registered
const SERVICE_FEE_PERCENT = 0.03; // 3% platform fee
...
export const ticketsRouter = router({
  ...
  createCheckoutSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // ── Build line items (no explicit type annotation for Stripe v22) ─
      const lineItems = [
        {
          price_data: {
            currency: "cad",
