...
export default function EventDetailPage({ citySlug, eventSlug }: Props) {
  ...
  const interestedCount =
    typeof event.interested === "number" && event.interested > 0
      ? event.interested
      : null;
  const organizerName = (event as any).organizerName ?? "LocalEvents Team";
  const isVerified =
    !!(event as any).isVerified || !!(event as any).organizerVerified;

  const isValidTag = (t: string) =>
    t.trim().length > 0 && !/^d+$/.test(t.trim());

  const tags: string[] = (() => {
    const raw = event.tags;
    if (!raw) return [];
    if (Array.isArray(raw))
      return (raw as string[]).map((t) => String(t).trim()).filter(isValidTag);
    const str = String(raw).trim();
    if (str.startsWith("[")) {
      try {
        const p = JSON.parse(str);
        if (Array.isArray(p))
          return p.map((t: unknown) => String(t).trim()).filter(isValidTag);
      } catch {
        /* fall */
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8]">
      ...
      <div
        ...
        style={{ aspectRatio: "16/9", maxHeight: 480, overflow: "hidden" }}
      >
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isVerified && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full">
                    <BadgeCheck className="w-3 h-3 fill-indigo-600 text-white" />
                    Verified
                  </span>
                )}
              </div>
                ...
                <section className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-xs text-gray-500 bg-gray-100 rounded-full px-3 py-1 capitalize"
                      >
                        #{tag}
                      </span>
                    ))}
                  ...
                  {[
                    {
                      ...
                      title: "Ticket Policy",
                      body: "All ticket sales are final. No refunds or exchanges unless the event is cancelled or rescheduled by the organizer.",
                    },
                    {
                      icon: CheckCircle2,
                      color: "text-green-500",
                      title: "Entry Requirements",
                      body: "Valid ID may be required. Tickets must be presented digitally or printed at the door.",
                    },
                    {
                      icon: AlertTriangle,
                      color: "text-amber-500",
                      title: "Age Restrictions",
                      body: "Some events may have age restrictions. Please verify event details before purchasing tickets.",
                    },
                    {
                      icon: Info,
                      color: "text-gray-400",
                      title: "Event Changes",
                      body: "The organizer reserves the right to make changes to the lineup or schedule.",
                    },
                    {
                      icon: Info,
                      color: "text-gray-400",
                      title: "Liability",
                      body: "LocalEvents acts as a platform connecting event-goers with organizers and is not liable for the conduct of events listed on this site.",
                    },
