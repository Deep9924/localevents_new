...
function getTicketCount(ticket: TicketItem): number {
  if ("quantity" in ticket && typeof (ticket as Record<string, unknown>).quantity === "number") {
    return Math.max(1, (ticket as Record<string, unknown>).quantity as number);
  }
  return 1;
}

function getPaymentAmount(ticket: TicketItem, eventPrice?: string | null): string {
  if ("amount" in ticket && typeof (ticket as Record<string, unknown>).amount === "string") {
    return (ticket as Record<string, unknown>).amount as string;
  }
  if ("total" in ticket && typeof (ticket as Record<string, unknown>).total === "string") {
...
function getPaymentMethod(ticket: TicketItem): string {
  if ("paymentMethod" in ticket && typeof (ticket as Record<string, unknown>).paymentMethod === "string") {
    ...
    <div className="w-[188px] shrink-0 snap-start rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:w-[210px]">
      ...
      <div className="mx-auto grid w-32 grid-cols-11 gap-[2px] rounded-xl bg-white p-2 ring-1 ring-slate-200 sm:w-36">
        {cells.map((filled, i) => (
          <div
            key={i}
            className={`aspect-square rounded-[1px] ${filled ? "bg-slate-900" : "bg-transparent"}`}
          />
        ))}
      </div>
...
function TicketDropdown({
  ...
  const paymentAmount = getPaymentAmount(ticket, "price" in ticket ? undefined : undefined);
      ...
      <div className="space-y-4 border-t border-slate-200 px-4 pb-4 pt-4">
        <div>
          ...
          <div className="-mx-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0">
            <div className="flex min-w-max gap-3 snap-x snap-mandatory sm:grid sm:min-w-0 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            ...
            <p className="text-sm font-semibold text-slate-900">{paymentAmount}</p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            ...
            <p className="text-sm font-semibold text-slate-900">{purchaseDate}</p>
          </div>
        </div>
      </div>
    </details>
