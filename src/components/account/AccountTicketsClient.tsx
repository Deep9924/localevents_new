...
import type { TicketFilter } from "@/server/db/tickets";
        ...
        <div className="px-4 py-3">
          ...
          {Number(ticket.taxAmount ?? 0) > 0 && (
            <div className="mb-1.5 flex items-center justify-between text-[12px]">
              ...
              <span className="text-slate-700">
                CAD {Number(ticket.taxAmount).toFixed(2)}
              </span>
            </div>
          )}
...
export default function AccountTicketsClient({
  initialTickets,
  initialFilter,
}: {
  ...
  const changeFilter = (nextFilter: TicketFilter) => {
    startTransition(() => {
      ...
      router.push(`/account/tickets${params.toString() ? `?${params.toString()}` : ""}`);
    });
    setExpanded({});
  };
              ...
              return (
                ...
                >
                  ...
                  {openPanel === "payment" && (
                    <>
                      <div className="mx-5 border-t border-slate-100" />
                      <PaymentPanel ticket={ticket as TicketWithEvent} />
                    </>
                  )}
