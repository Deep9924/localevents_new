...
export default function SearchPage() {
  ...
  const { data: categoryEvents = [] } = trpc.events.getByCity.useQuery(cityInput, {
    enabled: !!citySlug,
  });

  const { data: searchEvents = [] } = trpc.events.search.useQuery(searchInput, {
  ...
  const weekendEvents = useMemo(() => categoryEvents.slice(0, 6), [categoryEvents]);
